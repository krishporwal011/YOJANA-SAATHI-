...
import { Landmark, FileCheck2, ExternalLink, Clock } from "lucide-react";
...
export const SchemesPreview: React.FC = () => {
  ...
  useEffect(() => {
    const loadSchemes = async () => {
      ...
      setError(false);
      try {
        const res = await fetch(buildApiUrl("/api/schemes"));
        if (res.ok) {
    ...
    <div className="w-full max-w-5xl mt-10">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Validated against canonical scheme dataset
          </p>
        </div>
      ...
      {loading ? (
        <div className="text-center py-12 border border-slate-200 rounded-2xl bg-white">
          ...
          <p className="text-xs text-slate-500 font-medium">Loading verified schemes from backend...</p>
        </div>
      ) : error || schemes.length === 0 ? (
        <div className="text-center py-10 border border-amber-200 bg-amber-50 rounded-2xl p-6">
          ...
          <p className="text-xs text-amber-700">
            {isLocalDev
              ...
              : "The backend scheme service is currently warming up or unavailable. Please try again in a few moments or use our Eligibility Check tool."}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {schemes.map((scheme, idx) => (
            ...
            >
              ...
              <div>
                <div className="text-[10px] font-bold text-orange-600 tracking-wider uppercase mb-1 flex items-center justify-between">
                  <span>{scheme.ministry || "Government of India"}</span>
                  {scheme.id && <span className="text-slate-400 font-mono text-[9px]">{scheme.id}</span>}
                </div>
                ...
                <p className="text-xs text-slate-600 mb-4 line-clamp-3 leading-relaxed">
                  {scheme.benefits || "Official welfare benefit provided under this scheme."}
                </p>
