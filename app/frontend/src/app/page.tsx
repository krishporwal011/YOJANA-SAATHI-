...
import {
  ArrowRight, CheckCircle2, FileText, Landmark, LockKeyhole,
  Search, ShieldCheck, Sparkles, UserRound, WalletCards, ExternalLink, Mail, PhoneCall
} from "lucide-react";
...
export default function Home() {
  ...
  // Active Citizen Profile State (Default Demo User: Ramesh Kumar)
  ...
  const requireAuth = (targetView: MainView) => {
    if (isLoggedIn) {
      setView(targetView);
      if (targetView === "eligibility") setStep("profile");
  ...
  const handleLoginSuccess = (
    identifier: string,
    token: string,
    googleUser?: { name: string; email: string; isGoogleUser: true }
  ) => {
    setIsLoggedIn(true);
    setAuthToken(token);
    ...
    if (destination === "eligibility") setStep("profile");
    setNextTarget(null);
  };
  ...
  // -------------------------------------------------------------
  // RENDER: LOGIN VIEW (Authentication Guard Screen)
  // -------------------------------------------------------------
  if (view === "login") {
    return (
      <main className="min-h-screen bg-gradient-to-br from-[#061A41] via-[#08265B] to-[#0B2F6F] relative overflow-hidden flex flex-col justify-between">
        ...
        <Footer />
        <ChatbotModal isOpen={isChatOpen} onClose={() => setIsChatOpen(false)} userProfile={profile} />
      </main>
    );
  }

  // -------------------------------------------------------------
  // RENDER: MY PROFILE SCREEN (Edit Citizen Details)
  // -------------------------------------------------------------
  if (view === "profile") {
    if (!isLoggedIn) {
      setView("login");
        ...
        <div className="dashboard-container py-12 flex-1 flex items-center justify-center">
          <div className="bg-white border border-slate-200 rounded-2xl p-8 max-w-xl w-full shadow-xl">
            <div className="mb-6 flex items-center justify-between">
              ...
              <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full">CONTACT US</span>
            </div>
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h2 className="text-xl font-bold text-slate-900">Yojana Saathi AI</h2>
                <p className="text-xs text-slate-500">Government Scheme Assistant Portal</p>
              </div>
            </div>

            <p className="text-xs text-slate-600 mb-6 leading-relaxed">
              Have questions, feedback, or collaboration queries regarding government scheme eligibility matching? Reach out directly to our team via email below.
            </p>

            <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl mb-6">
              <span className="text-[11px] font-semibold text-slate-400 block mb-1">Official Email Address:</span>
              <a
  ...
  // -------------------------------------------------------------
  // RENDER: PROTECTED WORKFLOW (Eligibility Check & Results)
  // -------------------------------------------------------------
  if (view === "eligibility") {
    if (!isLoggedIn) {
      setView("login");
    ...
    return (
      <main className="min-h-screen workflow-page">
        ...
        <div className="workflow-shell">
          <div className="mb-4">
            <BackButton onClick={() => setView("home")} label="Back to Home" className="bg-white/10 text-white border-white/20 hover:bg-white/20" />
          </div>
          <div className="workflow-card">
            <div className="workflow-stepper">
              <ProgressSteps current={step} inHero={false} />
            </div>
            <section>
              {step === "profile" && <ProfileForm onNext={goToConfirm} initial={profile} />}
  ...
  // -------------------------------------------------------------
  // RENDER: PUBLIC SHOWCASE LANDING PAGE (Unauthenticated Default)
  // -------------------------------------------------------------
  return (
    <main className="min-h-screen dashboard-page">
      ...
      <section className="dashboard-hero">
        <div className="dashboard-container hero-grid">
          <div className="hero-copy">
            <div className="eyebrow"><Sparkles size={15} /> AI-POWERED CITIZEN SCHEME GUIDANCE</div>
            <h2>Find the right government schemes.<br /><span>Get the right support.</span></h2>
            <p>Check eligibility deterministically, understand required documents, and follow official application steps — all in one place.</p>
            
            {/* Hero Actions: ONLY Check Eligibility Now (Duplicate Ask Yojana Saathi Hero Button Removed) */}
            <div className="hero-actions">
              <button className="primary-cta" onClick={() => requireAuth("eligibility")}>
                <Search size={18} /> Check Eligibility Now <ArrowRight size={17} />
              </button>
            </div>
          </div>
            ...
            <div className="hero-art">
              <div className="trust-box">
                ...
                <div>
                  ...
                  <span>Your data is <em>100% confidential</em></span>
                </div>
              </div>
              ...
              <div className="legal-pill">
                <Landmark size={15}/> Official scheme information
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Honest Prototype Statistics (No unsupported fake numbers) */}
      <section className="stats-strip">
        <div className="dashboard-container stats-grid">
          <div className="stat-card">
            <WalletCards />
            <div><strong>30+</strong><span>Prototype schemes</span></div>
          </div>
          <div className="stat-card">
            <CheckCircle2 />
            <div><strong>Deterministic</strong><span>Eligibility engine</span></div>
          </div>
          <div className="stat-card">
            <FileText />
            <div><strong>Official</strong><span>Portal direct links</span></div>
          </div>
          <div className="stat-card">
            <ShieldCheck />
            <div><strong>AI Guidance</strong><span>Assistant support</span></div>
          </div>
        </div>
      </section>

      {/* Explore Popular Schemes Grid */}
      <section id="schemes" className="dashboard-container content-section">
        <div className="section-heading">
          <div>
            <span>EXPLORE</span>
            <h3>Popular Schemes</h3>
            <p>Explore official welfare schemes available for Indian citizens.</p>
          </div>
          <button onClick={() => requireAuth("eligibility")}>
        ...
        <div className="assistant-card">
          <div className="assistant-copy">
            ...
            <div>
              ...
              <h3>Need help finding a scheme?</h3>
              <p>Ask about eligibility, required documents, benefits, or application steps. The AI assistant will guide you based on verified scheme data.</p>
            </div>
          </div>
          <button className="primary-cta" onClick={() => setIsChatOpen(true)}>
            Open Ask Yojana Saathi <ArrowRight size={18} />
          </button>
        </div>
      </section>
